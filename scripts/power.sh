...
case $OPT in
	...
	# suspend) [ "$(printf "No\\nYes" | dmenu -i -fn 'NotoMonoRegular:bold:pixelsize=16' -p "Really suspend?")" = "Yes" ] && sleep 5 ; systemctl suspend ;;
	# suspend) [ "$(printf "No\\nYes" | dmenu -i -fn 'NotoMonoRegular:bold:pixelsize=16' -p "Really suspend?")" = "Yes" ] && ~/scripts/lock.sh && echo mem > /sys/power/state ;;
