...
open=("$(wmctrl -l | awk '{print $2, " : ",  $NF, " - ", $4}' | sort -V -k 1 | column -tc2 | awk '!/-1/' | awk '!/wsh/')
kill
cancel")

close=("$(wmctrl -l | awk '{print $1, $NF, $4, " - ",  $2}' | sort -V -k 4 | column -tc2 | awk '!/-1/' | awk '!/wsh/')
cancel")

# workspace="$(echo "${open[@]}" | dmenu -i -c -l 10 -nb '#222222' -nf '#b8b8b8' -sf '#181818' -sb '#007687' -p "Open Windows:")"
...
if [ "$workspace" == "kill" ];
then
    # closewin="$(echo "${close[@]}" | dmenu -i -c -l 10 -nb '#222222' -nf '#b8b8b8' -sf '#181818' -sb '#007687' -p "Which would you like to close?: ")";
    closewin="$(echo "${close[@]}" | fzf --prompt="Close Window" --border=rounded --margin=5% --color=dark --height=100% --reverse --header="    Windows per Workspace" --info=hidden --header-first)"
    if [ "$closewin" == "cancel" ];
    then
        exit
